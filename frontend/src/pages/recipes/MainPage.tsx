...
const MainPage: React.FC = () => {
  ...
  const sortRecipes = useCallback(
      (recipes: Recipe[]): Recipe[] => {
        return recipes.sort((a, b) => {
          if (sortOrder === "asc") {
            return a.cooking_time - b.cooking_time || a.title.localeCompare(b.title);
          } else {
            return b.cooking_time - a.cooking_time || a.title.localeCompare(b.title);
          }
        });
      },
      [sortOrder]
  ...
  const fetchRecipes = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8080/api/recipes-by-filters`, {
        params: {
        ...
        <div className="mx-[15vw]">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
            ...
            <div className="ml-4 mt-4 sm:mt-0">
              <RecipeTypeFilter selectedTypes={selectedTypes} onChange={setSelectedTypes} />
            </div>
          </div>
          ...
          {selectedTypes.length > 0 && <div className="mb-4">{getFilteredDescriptions()}</div>}
